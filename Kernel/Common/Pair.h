...
public:
    ...
    Quad(FirstT&& first, SecondT&& second, ThirdT&& third, FourthT&& fourth)
        : m_first(forward<FirstT>(first)), m_second(forward<SecondT>(second)),
          m_third(forward<ThirdT>(third)), m_fourth(forward<FourthT>(fourth)){ }
...
auto make_quad(First&& first, Second&& second, Third&& third, Fourth&& fourth)
{
    ...
    using QuadType = Quad<remove_reference_t<First>, remove_reference_t<Second>, remove_reference_t<Third>, remove_reference_t<Fourth>>;
